...
| [audit](audit/) | is anything unused or misconfigured? | a scan that encodes the false alarms, so ghosts don't get chased twice |
| [perfmode](perfmode/) | max performance on demand on Threadripper | there's nothing to tune: EPP is the only knob, and it's a bias, not a lock |
